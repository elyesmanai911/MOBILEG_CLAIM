...
public class ListProductsForm extends BaseFormBack {
    ...
    public ListProductsForm(Form previous,Resources res) throws IOException {
        ...
        ArrayList<Categorie> categ;
         
        categ = new ArrayList<>();
        categ = ServiceCategorie.getInstance().getAllOeuvres();
         ArrayList<Produit> list;
         
        list = new ArrayList<>();
        list = ServiceProduit.getInstance().getAllOeuvres();
        for ( Categorie p : categ) {
                      RadioButton rb1 = new RadioButton(p.getNom_categorie());
                      new ButtonGroup(rb1);
                      addAll(rb1);
                      rb.add(rb1);
                  }
        for (RadioButton r :rb){
                 ...
                 r.addActionListener((evt) -> {
                     ArrayList<Produit> list1;
        list1 = new ArrayList<>();
        list1 = ServiceProduit.getInstance().filtreProd(r.getText());
        
                     try {
                         new ListProductsFilterForm(this.getComponentForm(),res, list1).show();
        ...
        for ( Produit p : list) {
         ...
         Label spl5 = new Label("Product Views: " + "  " + p.getNbr_vu());
         spl5.getAllStyles().setFgColor(0x00000);
         Label spl6 = new Label("Product qte: " + "  " + p.getQte_produit());
