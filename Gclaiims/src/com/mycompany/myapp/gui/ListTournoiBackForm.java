 ...
 * @author souma
 */
public class ListTournoiBackForm extends BaseFormBack{
public ArrayList<Tournoi> Tournoi;
    public ListTournoiBackForm(Resources res) {
        super("ListCoach", BoxLayout.y());
  super.addSideMenu(res);
  Tournoi=ServiceTournoi.getInstance().getAllHotels();
Label labe=new Label("LISTE DES Tournois ");
addStringValue("",labe);
int i=1;
for (Tournoi e :Tournoi)
{ Label l=new Label("Tournoi N°"+i++);
l.getAllStyles().setFgColor(0xFFA500);
addStringValue("",l);

Label xx =new Label("NOM Du Tournoi");
xx.getAllStyles().setFgColor(0x00000);
Label qq =new Label("Description");
qq.getAllStyles().setFgColor(0x00000);


addStringValue(e.getNomTournoi(),xx);
addStringValue(e.getDescription(),qq);

 Label Modiff =new Label("Modifier un Tournoi");
...
Modiff.addPointerPressedListener(ll->new ModifTournoiForm(res,e).show());  
      Label Modif =new Label("Supprimer un Tournoi");
...
Modif.addPointerPressedListener(new ActionListener() {
       public void actionPerformed(ActionEvent evt) {
                    ...
                    ServiceTournoi.getInstance().Supprimer(e.getId());

                };

            
       } );
Modif.addPointerPressedListener(ll->new ListTournoiBackForm(res).show());  
}
